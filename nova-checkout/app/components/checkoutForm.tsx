...
const CheckoutForm : React.FC<CheckoutFormProp> = () => {
  ...
  const [selectPaymentWallet, setSelectedPaymentWallet] = useState<Wallet>("");
  const [selectedReceivingWallet, setReceivingWallet] = useState<Wallet>("")
    ...
    <div className='bg-white border border-slate-100 rounded-2xl p-8'>
      <nav className='flex gap-x-3 gap-y-4  bg-[#F2F2F2] rounded-2xl mb-8'>{
        checkoutOptions.map((option, index)=>(
          <div key={index + option} className='bg-[#F2F2F2] text-[#828282] rounded-2xl px-2 py-1 hover:bg-[#013941] hover:text-white cursor-pointer'>
            {option}
          </div>
        ))}</nav>
            ...
            <div className='w-fit flex flex-col'>
              ...
              <div>
                ...
                isNetworkDropdownOpen && (
                  ...
                  <div className='absolute bg-white rounded-2xl -left-24 top-1 border border-[#E0E0E0] p-2'>
          ...
          <div>
            <div className='w-full flex flex-col'>
              <button
                ...
                id="dropdownUsersSearchButton" data-dropdown-toggle="dropdownSearch" data-dropdown-placement="bottom" 
                className="inline-flex w-full items-start self-end justify-start text-black bg-brand box-border border border-[#E0E0E0] rounded-2xl shadow-xs font-medium leading-5 rounded-base text-sm px-4 py-2.5 focus:outline-none" type="button">
                {selectedCurrency.code}
                <svg className="w-4 h-4 ms-1.5 -me-0.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m19 9-7 7-7-7"/></svg>
              </button>
              <div className="relative inline-block "></div>
